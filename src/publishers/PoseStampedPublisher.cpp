...
PoseStampedPublisher::PoseStampedPublisher(sbp::State* state,
    ...
    : SBP2ROS2Publisher<geometry_msgs::msg::PoseStamped,
                        sbp_msg_pos_ecef_t, sbp_msg_orient_euler_t>(
          state, topic_name, node, logger, enabled, frame) {}

void PoseStampedPublisher::handle_sbp_msg(
    uint16_t sender_id, const sbp_msg_pos_ecef_t& msg) {
  (void)sender_id;


 // OBS msg has not arrived yet.
  if (last_received_orient_euler_tow_ == 0) {
    LOG_WARN(logger_, "Orient euler message has not arrived yet. Not publishing");
    return;
  }
  ...
  if ( time_diff > MAX_ORIENT_EULER_TIME_DIFF_ ) {
    LOG_WARN(logger_, "Time difference between ORIENT EULER message and POS ECEF message is larger than Max");
    return;
  }
...
void PoseStampedPublisher::handle_sbp_msg(
    uint16_t sender_id, const sbp_msg_orient_euler_t& msg) {
  (void)sender_id;
